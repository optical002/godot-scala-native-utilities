...
class RunLcaSuite extends munit.FunSuite:
  ...
  test("empty / dummy config does nothing") {
    val hp = Health(100.0)
    Config.dummy[LcGameEntity].runLca(entity(Player(hp)), entity(Enemy(Health(100.0))))
